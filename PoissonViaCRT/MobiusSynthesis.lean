...
The lattice-point counting and small-divisor machinery has been extracted to
`SmallDivisorHelpers.lean`, and the Euler weight bounds to `EulerWeights.lean`.
...
deviation vanishes for any tuple `h`.
-/
