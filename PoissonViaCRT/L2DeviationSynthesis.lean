...
private lemma variance_factorization (ε : ℝ) (k : ℕ) (Ω : ∀ p : ℕ, Finset (ZMod p))
    ...
    C_box * s ^ (k - 1 : ℕ) * ∏ p ∈ T, ((combinedEulerWeight ε k Ω p * localMean k Ω p)^2 + (k : ℝ)^2 * (C_gamma / (p : ℝ))) := by
  sorry

/-- **Step 5: Absorption bound.** The product `∏_{p ∈ T} ((cEW_p · μ_p)² + k² · C_gamma / p)` is bounded by
a constant `K` times `(∏_{p ∈ T} (cEW_p · μ_p))²`.
Note: we absorb `k² · C_gamma / p`, NOT `p²`. Since `(cEW·μ)² ∼ p^{2-2ε}` and `2-2ε > 1` for `ε < 1/2`,
the ratio decays, so the product `∏_T (1 + (k²·C_gamma/p)/(cEW·μ)²)` converges. -/
private lemma variance_product_absorption (ε : ℝ) (hε : 0 < ε) (k : ℕ) (hk : 2 ≤ k)
    ...
    ∏ p ∈ T, ((combinedEulerWeight ε k Ω p * localMean k Ω p) ^ 2 + (k : ℝ)^2 * (C_gamma / (p : ℝ))) ≤
...
private lemma variance_per_T_bound (ε : ℝ) (hε : 0 < ε) (k : ℕ) (hk : 2 ≤ k)
  ...
  calc ∑ h ∈ S, (∏ p ∈ T, (localCount Ω q (Fin.cons 0 (fun i => (h i : ZMod q))) p -
      ...
      _ ≤ ∑ h ∈ S, ∏ p ∈ T, (A p + B p h) := by
        ...
        intro p hp
        exact local_variance_pointwise_bound ε k hk Ω hWD q p (hT_sub hp) h
      ...
      _ ≤ ∑ U ∈ T.powerset, (∏ p ∈ T \ U, A p) *
        ...
        rw [mul_assoc]
        have H : ∑ h ∈ S, ∏ p ∈ U, B p h ≤ (∏ p ∈ U, (k : ℝ)^2) * (C_box * s ^ (k - 1 : ℕ) * ∏ p ∈ U, (C_gamma / (p : ℝ))) := by
          calc ∑ h ∈ S, ∏ p ∈ U, B p h
            ...
            _ = (∏ p ∈ U, (k : ℝ)^2) * ∑ h ∈ S, ∏ p ∈ U, (if Function.Injective (Fin.cons (0 : ZMod p) (fun i => (h i : ZMod p))) then (0:ℝ) else 1) := by
              rw [← Finset.mul_sum]
            _ ≤ (∏ p ∈ U, (k : ℝ)^2) * (C_box * s ^ (k - 1 : ℕ) * ∏ p ∈ U, (C_gamma / (p : ℝ))) := by
              gcongr
